...
function footer(){
 $footer = "<div class='footer'><a href='".HOST."/about/stats.php'>Stats</a><div style='float:right;'>&copy; Copyright <a target='_blank' href='http://www.saguneshgrover.com'>Sagunesh Grover</a>".date("Y")."</div></div>";
 $footer.='
...
function getResults(){
 ...
 $dbh  = new PDO('mysql:dbname='.$db.';host='.$host.';port='.$port, $user, $pass);
  ...
  $sql=$dbh->prepare("SELECT `title`, `url`, `description` FROM search WHERE `title` LIKE :q OR `url` LIKE :q OR `description` LIKE :q ORDER BY id");
  $sql->bindValue(":q", "%$q%");
  $sql->execute();
